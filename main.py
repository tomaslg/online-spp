...
from plot_util import plot_distribution,plot_histogram_sigma,get_plot,get_graph_instance_1,get_graph_instance_2,plot_regret_t
...
output_dir=os.path.join(os.path.dirname(os.getcwd()),"output6")
...
for (_reversed__,one_border,borders_faster,obstacle_slower,snake_opt_path
    ...
    for norm_ in norm_range_:
        Theta__ = {kernel_name : multipliers_theta for kernel_name in Kernels.keys()
            ...
            for __theta_p in Theta__[kernel_name]:
                ...
                local_theta_neighborhood = [
                    ...
                    +f"_{-1/6. + float(__theta_p.split('_')[1])}")] if any(
                        [one_border,borders_faster,obstacle_slower,snake_opt_path]
                ...
                #T_iter=2
                    # N=3
                        ...
                        max_x_y= (  max( [centers_[i][0] for i in range(len(A))] ), 
                                  max( [centers_[i][1] for i in range(len(A))] )) 
                        min_x_y= (  min( [centers_[i][0] for i in range(len(A))] ), 
                                  min( [centers_[i][1] for i in range(len(A))] )) 
                        ...
                        if pp:pp=PdfPages(os.path.join(output_dir,
                            ...
                            f"{'_reversed' if _reversed__ else ''}_{kernel_name}"+
                        ...
                        if Plot_Instance_1:
                            edge_color_instance_1=['r'  if int( centers_[i][0]/Delta )%2==
                                                            int( centers_[i][1]/Delta )%2 else 'b' 
                                                            for (i,a) in enumerate(A)]
                            # get_plot(G=G_2,edge_color_=edge_color_instance_1,pp=pp,plt_=plt,Title="Instance")
                            chessboard_truth= np.random.random(len(A))*.1 + np.array(
                            [ np.log(v_real*(
                                .1 if int( centers_[i][0]/Delta )%2 == int( centers_[i][1]/Delta )%2 
                                else .9
                                )) for (i,a) in enumerate(A) ])
                        ...
                        #    color_index = color_keys[np.random.randint(len(mcolors.CSS4_COLORS))]
                            if borders_faster:
                                for i in range(N+1):
                                    for i_a,a in enumerate(A):
                                        if (((centers_[i_a][0]==i*.5 or centers_[i_a][0]==(N-1- i*.5)) and i*.5<=centers_[i_a][1]<=(N-1- i*.5)) or 
                                            ((centers_[i_a][1]==i*.5 or centers_[i_a][1]==(N-1- i*.5))  and i*.5<=centers_[i_a][0]<=(N-1- i*.5))):
                                            V_arcs_dict[a]=np.log(v_real * (2/(1 + 2**.5 ))**(i-2) )
                            ...
                            #        Sampled_color_indices.append(color_index)
                                    ...
                                    V_arcs_dict[a] = np.log(
                                        v_real * ((1 + 2**.5 )/2)**(
                                            max(orientation_effect[0]*max(
                                                centers_[i_a][0]-slow_gravity_center[0],
                                                slow_gravity_center[0]-centers_[i_a][0]),
                                                orientation_effect[1]*max(
                                                centers_[i_a][1]-slow_gravity_center[1],
                                                slow_gravity_center[1]-centers_[i_a][1]) 
                                                )-60))
                                ...
                                for i_a,a in enumerate(A):
                                     V_arcs_dict[a] = np.log(v_real*(.1 if int( 
                                        centers_[i_a][0]/Delta )%2 == int( 
                                            centers_[i_a][1]/Delta )%2 else .9)) 
                            ...
                            instance__= np.array( [V_arcs_dict[a] for a in A ] )
                    
                        get_plot(G=G_2,edge_color_=-instance__,pp=pp,plt_=plt,
                                 Title="Instance")    
                        ...
                        prior_NormalIG = {}
                        prior_NormalIG["mu"] = np.ones(len(A)) * np.log(v_prior)
                        prior_NormalIG["kappa"] = np.ones(len(A)) 
                        prior_NormalIG["alpha"] = np.ones(len(A)) *_alpha__
                        prior_NormalIG["beta"] = np.ones(len(A)) * _beta__
                        
                        
                        prior_Spatial = {}
                        prior_Spatial["mu"] = np.ones(len(A)) * np.log(v_prior)
                        prior_Spatial["kappa"] = np.ones(len(A)) 
                        prior_Spatial["alpha"] = np.ones(len(A)) *_alpha__
                        prior_Spatial["beta"] = np.ones(len(A))  * _beta__
                        prior_Spatial["theta"] = general_multiplication(__theta_,
                                                                        dist__)
                        prior_Spatial["rho"] = {}
                        prior_Spatial["phi"] = np.zeros((len(A), len(A)))#
                        # prior_Spatial["phi"]=dok_matrix((len(A), len(A)), dtype=np.float64)
                        prior_Spatial["name"] = kernel_name+"_"+str(prior_Spatial["theta"])
                        prior_Spatial["reversed"] = _reversed__
                        # prior_Spatial["influential_arcs"]={}
                        # kernel = exponential
                        
                        
                        if not any([one_border,borders_faster,obstacle_slower,snake_opt_path]):
                            for i in range(len(A)):
                            ...
                            #        i==j or not (centers_[i][1]-centers_[j][1]==0 and ): continue
                                    rho_ij=np.linalg.norm(centers_[i]*dist__-centers_[j]*dist__,norm_)
                                    if type(prior_Spatial["theta"]) is str:
                                        if not (
                                                np.abs(A[i][1][0]-A[i][0][0])
                                                ==np.abs(A[j][1][0]-A[j][0][0]) 
                                                or np.abs(A[i][1][1]-
                                                          A[i][0][1])==np.abs(A[j][1][1]-
                                                                              A[j][0][1])
                                                ):
                                            prior_Spatial["phi"][i,j]= kernel(float(
                                                prior_Spatial["theta"].split("_")[1]),
                                                rho_ij)
                                        else:
                                            prior_Spatial["phi"][i,j] = kernel(float(
                                                prior_Spatial["theta"].split("_")[0]),
                                                rho_ij)
                                    else:
                        ...
                        plot_histogram_sigma(non_corrated_dist,pp,plt,"Histogram_sigma2_before_training")
                        plot_histogram_sigma(spatial_distribution,pp,plt,"Histogram_sigma2_before_training")
                        plot_distribution(non_corrated_dist,pp,plt,"mu_density_before_training")
                        plot_distribution(spatial_distribution,pp,plt,"mu_density_before_training")
                        (regret_,Paths_,Observed_values_,
                         Paths_expert,mu_updates,Time_
                         )=TS_Stationary(T_iter,G_2,instance__,sigma,d_all_arcs,
                                         [non_corrated_dist,spatial_distribution],
                        ...
                        for d_ in  range(len(regret_)):
                            ...
                            for name__,_pths in Paths_[d_]:
                                kernel_iteration,regret_printted = name__.split(", ")
                                dist_name,t_ = kernel_iteration.split("=")
                                dist_name = dist_name.replace("_Iter","")
                                t_ = int(t_.split("/")[0])
                                registered_results["distribution"].append(dist_name)
                                registered_results["benchmark"].append(kernel_name)
                                ...
                                registered_results["regret"].append(regret_[d_][t_-1])
                                # for i in range(len(A)):
                        ...
                        plot_histogram_sigma(non_corrated_dist,pp,plt,"Histogram_sigma2_after_training")
                        plot_histogram_sigma(spatial_distribution,pp,plt,"Histogram_sigma2_after_training")
                        plot_distribution(non_corrated_dist,pp,plt,"mu_density_after_training")
                        plot_distribution(spatial_distribution,pp,plt,"mu_density_after_training")
                        ...
                        if Plot_Instance_1:
                            ...
                            edge_color_instance_2=[ ((-instance__[i]
                                                      #'r'  if int( centers_[i][0]/Delta )%2==
                                                            # int( centers_[i][1]/Delta )%2 else 'b' 
                                                            ) if A[i] not in Paths_expert[-1][1] else -N/2#'k'
                                                     ) for i in range(len(A))]
                            get_plot(G=G_2,edge_color_=edge_color_instance_2,pp=pp,
                        ...
                        if Plot_Instance_2:
                            ...
                            edge_color_instance_2=[ (-instance__[i]#_edge_color_instance_2[i] 
                                                     if A[i] not in Paths_expert[-1][1] else -instance__[i]#'k'
                                                     ) for i in range(len(A))]
                            get_plot(G=G_2,edge_color_=edge_color_instance_2,
                                     pp=pp,plt_=plt,Title=f"{Paths_expert[-1][0]}",
                                     _paths_=Paths_expert[-1][1])
                    
                    
                    
                        
                        plot_regret_t(prior_Spatial["name"],regret_NormalIG,
                ...
                if output_data_file:
                    (pd.DataFrame.from_dict(registered_results)).to_csv(os.path.join(output_dir,f"SIMULATION_Regret_{kernel_name}_norm{norm_}{'_reversed' if _reversed__ else ''}"+
                        ("_two_border" if borders_faster else (
                            "_one_border" if one_border else (
                            "_with_obstacle" if obstacle_slower else (
                            "_chessboard" if not snake_opt_path else 
