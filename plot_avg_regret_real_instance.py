...
#         }
    ...
    for _color, regret_Spatial, name in zip(colors_, regret_Spatial_, prior_Spatial_name): 
        plt_.plot((list(range(1,len(regret_Spatial[1])+1))), 
                 ...
                 f'{format_distrib_name(name.replace("Naive", greedy_name))}',#' = {np.round(regret_Spatial[1][-1],2)}', 
                 color=_color)
...
if __name__ == "__main__":
    ...
    # fl_ = f"consolidated_all_regrets_T{T}_N{len(files)}.csv"
    # regret_all_realizations.to_csv(os.path.join(
    #     output_dir, fl_), index=False)
    # regret_all_realizations = pd.read_csv(os.path.join(
    #     output_dir, fl_))
    ...
    for regret_val in approaches_instance_convergence.keys():
        ...
        for c_ in approaches_keys:
            print(approaches_instance_convergence[regret_val][c_],
                  ...
                  ",", approaches_instance_sum_total_regret[regret_val][c_],#approaches_instance_not_converged[regret_val][c_],
